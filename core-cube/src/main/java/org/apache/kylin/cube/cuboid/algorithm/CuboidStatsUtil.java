...
public class CuboidStatsUtil {
    ...
    /**
     * The formula for calculating the benefit of cube optimization according to recommended cuboids is as follows:
     ...
     * @return the benefit of cube optimization according to recommended cuboids
     */
    public static OptimizationBenefit calculateOptimizationBenefit(Map<Long, Long> statistics,
            Map<Long, Double> cuboidsSize, CuboidScheduler curCuboidScheduler, CuboidScheduler recomCuboidScheduler,
            Map<Long, Double> cuboidHitProbabilityMap, double spaceLimit, double spaceBenefitRatio) {
        if (cuboidHitProbabilityMap == null || cuboidHitProbabilityMap.isEmpty()) {
        ...
        {// compute the benefits of added cuboids in recommendCuboids
            ...
            for (Long cuboid : recommendCuboidsOnly) {
                ...
                long rollupCount = statistics.get(pCuboid) - statistics.get(cuboid);
                rollupBenefit += rollupCount * cuboidHitProbabilityMap.get(cuboid);
            }
        }
        ...
        {// compute the cost of removed cuboids in currentCuboids
            ...
            for (Long cuboid : currentCuboidsOnly) {
                Long pCuboid = recomCuboidScheduler.findBestMatchCuboid(cuboid);
                long rollupCount = statistics.get(pCuboid) - statistics.get(cuboid);
                rollupCost += rollupCount * cuboidHitProbabilityMap.get(cuboid);
            }
        }
        ...
        {// compute the total rollup input count
            for (Long cuboid : currentCuboids) {
                Long rowCount = statistics.get(cuboid);
                rollupInputCount += rowCount * cuboidHitProbabilityMap.get(cuboid);
            }
        ...
        return new OptimizationBenefit(rollupBenefit, rollupCost, rollupInputCount, curTotalSize, recomTotalSize,
