 ...
 */
public abstract class TopNCounterSummaryBase<T> implements ITopNCounter<T> {
     ...
     */
    public TopNCounterSummaryBase(int capacity) {
     ...
     */
    protected abstract void sortUnsorted(int newCapacity);
    ...
    /**
     * It's for the merge process to estimate the count of none existing elements
     *
     * @return
     */
    ...
    public ITopNCounter<T> merge(ITopNCounter<T> another0) {
        ...
        return merge(another, m1, m2);
    }

    protected ITopNCounter<T> merge(TopNCounterSummaryBase<T> another, double m1, double m2) {
